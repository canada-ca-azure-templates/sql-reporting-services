...
function Get-TargetResource
{
    ...
    if(Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\RS" -Name $InstanceName -ErrorAction SilentlyContinue)
    {
        ...
        $RSConfig = Invoke-Command -ScriptBlock {
            $SQLVersion = $args[0]
            $InstanceName = $args[1]
...
function Set-TargetResource
{
    ...
    if(Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\RS" -Name $InstanceName -ErrorAction SilentlyContinue)
    {
        Invoke-Command -ScriptBlock {
            ...
            #0 = Windows, 1=SQL Server, 2 = Windows Service
            $connectionType = 2 
            ...
            $ReportManagerApp = "ReportManager"
            [int]$SQLVersionNum = [convert]::ToInt32($SQLVersion)
            if($SQLVersionNum -gt 12)
            {
                $ReportManagerApp="ReportServerWebApp"
            }

            
            if($RSConfig.VirtualDirectoryReportManager -ne $RMVirtualDirectory)
            {
                $RSConfig.SetVirtualDirectory($ReportManagerApp,$RMVirtualDirectory,$Language)
                $RSConfig.ReserveURL($ReportManagerApp,"https://+:443",$Language)
            }
            ...
            #MAYBE RUN THIS FROM PS AND SEE OUTPUT
            if($SQLCmdPath -ne "")
            {
                & "$SQLCmdPath" -S $RSConnection -i $DBCreateFile #-U $SQLAdminCredential.UserName -P $SQLAdminCredential.GetNetworkCredential().Password
                & "$SQLCmdPath" -S $RSConnection -i $DBRightsFile #-U $SQLAdminCredential.UserName -P $SQLAdminCredential.GetNetworkCredential().Password
                $RSConfig.SetDatabaseConnection($RSConnection,$RSDatabase,$connectionType,$SQLAdminCredential.UserName,$SQLAdminCredential.GetNetworkCredential().Password)
                $RSConfig.InitializeReportServer($RSConfig.InstallationID)
            }
            else
            {
		# if doesn't find cmd in the registry try to execute it , if is installed it would be in the environment
                & sqlcmd -S $RSConnection -i $DBCreateFile #-U $SQLAdminCredential.UserName -P $SQLAdminCredential.GetNetworkCredential().Password
                & sqlcmd -S $RSConnection -i $DBRightsFile #-U $SQLAdminCredential.UserName -P $SQLAdminCredential.GetNetworkCredential().Password
                $RSConfig.SetDatabaseConnection($RSConnection,$RSDatabase,$connectionType,$SQLAdminCredential.UserName,$SQLAdminCredential.GetNetworkCredential().Password)
                $RSConfig.InitializeReportServer($RSConfig.InstallationID)
            }
